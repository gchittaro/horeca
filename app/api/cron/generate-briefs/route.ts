...
async function generateBrief(profil: Record<string, unknown>, indicateurs: unknown[], signaux: unknown[], semaine: number, annee: number): Promise<string> {
  const volumes = [
  ...
  const indText = (indicateurs as { nom: string; valeur: number; unite: string; variation_pct: number }[]).map(i =>
    `${i.nom} : ${i.valeur} ${i.unite} (${i.variation_pct > 0 ? '+' : ''}${i.variation_pct.toFixed(1)}%)`
  ).join('\n')

  const sigText = (signaux as { titre: string; impact: string; horizon: string }[]).map(s =>
...
export async function GET(request: Request) {
  ...
  const { data: indicateurs } = await admin.from('indicateurs').select('nom, valeur, unite, variation_pct, categorie').eq('semaine', semaine).eq('annee', annee)
  const { data: signaux } = await admin.from('signaux_geopolitiques').select('titre, impact, horizon').order('fetched_at', { ascending: false }).limit(3)
  ...
  for (const user of users || []) {
    ...
    const { data: profil } = await admin.from('etablissements').select('type_etablissement, region, couverts_par_jour, vol_cafe, vol_viandes, vol_laitiers, vol_farine, vol_huiles, vol_energie').eq('user_id', user.id).single()
    if (!profil) continue

    try {
      const brief = await generateBrief(profil as Record<string, unknown>, indicateurs || [], signaux || [], semaine, annee)
      if (brief) {
      ...
      console.error(`[generate-briefs] user ${user.id}:`, e)
      errors.push(user.id)
    }
  }

  return NextResponse.json({ ok: true, generated, errors: errors.length ? errors : undefined })
}
