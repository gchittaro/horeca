                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  {inds.map(ind => {
                    ...
                    const periodeLabels: Record<string, string> = {
                      ...
                      annuel: 'vs an préc.', trimestr: 'vs trim. préc.',
                    }
                    const periodeLabel = ind.periode ? (periodeLabels[ind.periode] ?? null) : null
                    return (
                      <div key={ind.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '10px 12px', background: '#F8F8FC', borderRadius: 9 }}>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: 4 }}>
                          ...
                          {periodeLabel && <span style={{ fontSize: 9, color: '#B0AED6' }}>{periodeLabel}</span>}
                        </div>
                      </div>
                    )
                  })}
